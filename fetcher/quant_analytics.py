...
def compute_risk_adjusted_metrics(
    ...
    return {
        ...
        "calmar_ratio": round(calmar, 4) if not np.isnan(calmar) else "-",
        ...
        "gain_to_pain_ratio": round(gain_to_pain, 4) if not np.isnan(gain_to_pain) else "-",
        "profit_factor": round(profit_factor, 4) if not np.isnan(profit_factor) else "-",
...
def compute_benchmark_relative_metrics(
    ...
    return {
        ...
        "capture_ratio": round(capture_ratio, 2) if not np.isnan(capture_ratio) else "-",
...
def run_monte_carlo_simulation(
    ...
    np.random.seed(42)  # Deterministic seed for reproducible analysis
    shocks = np.random.normal(0, 1, size=(n_simulations, n_days))
